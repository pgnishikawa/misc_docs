...
**用途の例**：
- 工場出荷時に、量産用の書き込み治具（外部 I²C ライタ）でまとめて書き込む
- あるいは RZ/N2L のファームウェア自身に「初回起動時に EEPROM が未設定なら
  上記手順で書き込む」というプロビジョニング処理を実装する
- フィールドでの ESI/設定更新（Vendor情報の変更や不具合修正）も同じ経路で可能

いずれの方式を採るかは製品要件次第ですが、**この書き込み経路自体はソフトウェアだけで
完結できる**ということは設計初期に知っておくべき事実です。
