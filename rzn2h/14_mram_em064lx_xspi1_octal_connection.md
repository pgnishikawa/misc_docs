...
|---|---|---|---|
| データ線 | IO0-7（8本） | `XSPI1_IO0`-`XSPI1_IO7` | ○ |
| クロック | **CK（単相、1本のみ）**。24-BGAのボール表に相補クロック(CK#)は存在しない | `XSPI1_CKP`（+ `XSPI1_CKN`、差動ペア） | **`XSPI1_CKP`をMRAMの`CK`へ接続。`XSPI1_CKN`はMRAM側に接続先がない（NC）**（要ハードウェア確認、§7） |
| データストローブ | `DS`（出力） | `XSPI1_DS` | ○ |
| チップセレクト | `CS#` | `XSPI1_CS0#` | ○ |
| リセット | `RESET#`（入力、BGAでは専用ピン。**外部プルアップ推奨**とMRAM DSに明記） | `XSPI1_RESET0#` | ○（x8ブート時と同じ配線パターン、[09](09_xspi0_x1_boot_and_runtime.md) §1.1参照） |
| 電源電圧 | **単一1.8V**（3.3V動作の記載なし） | `VCC1833_5`（xSPI1用ドメイン）を**1.8Vに設定** | **1.8V固定が必須**（3.3Vでは動作しない可能性が高い。RZ/N2H側は1.8V/3.3V選択可能なドメインなので、確実に1.8V側を選ぶこと） |
...
- **書き込み完了レイテンシの正式値**: MRAM DSの電気特性表に明示的な「Page Program time」
  ...
  書き込み直後リードバック検証を推奨する。
- **`XSPI1_CKN`（差動クロックの負側）を未接続のままにしてよいか**: MRAM側が単相クロック
  仕様であることは確認したが、RZ/N2Hのxsゲート出力ドライバがCKN未接続時にCKP単体で
  規定通りのタイミング・信号品質を出せるかは、RZ/N2Hハードウェアマニュアルの電気的
  特性章、または実機の信号品質測定で確認すること。
...
- [ ] `XSPI1_CKP`をMRAMの`CK`へ接続し、`XSPI1_CKN`の扱い（NC/終端）をハードウェア設計側で確認した
- [ ] `XSPI1_RESET0#`を接続し、MRAM側`RESET#`に外部プルアップを設けた
